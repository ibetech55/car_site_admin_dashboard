...
import { CREATE_MODEL_CATEGORY, GET_MODEL_CATEGORIES, GET_MODEL_CATEGORY_LIST, modelCategoryActions } from './model.category.action';
...
export class ModelCategoryEffects {
  constructor(private action$: Actions, private _modelCategoryService: ModelCategoryService) {}
  ...
  _createModelCategpry = createEffect(() =>
    this.action$.pipe(
      ...
      switchMap((action:{values:ICreateModelForm}) => {
        return this._modelCategoryService.createModelCategory(action.values).pipe(
          map((data) => {
